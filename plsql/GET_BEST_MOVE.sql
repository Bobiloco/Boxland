    ...
    select ec.choice_facing, 
           CASE objCD 
             ...
             WHEN 4 THEN CASE WHEN ( NVL(choiceDir.sumpos,0) <> 0 ) AND ( NVL(choiceDir.sumpct,0) <> 0 ) 
                       ...
                       + CASE WHEN ( NVL(eventLoc.sumpos,0) <> 0 ) AND ( NVL(eventLoc.sumpct,0) <> 0 ) 
                              THEN ( NVL(eventLoc.pos,0) * NVL(eventLoc.pct,0) ) / ( NVL(eventLoc.sumpos,0) * NVL(eventLoc.sumpct,0) ) 
                                      * ( NVL(choiceDir.sumpos,0) + NVL(eventLoc.sumpos,0) + NVL(choiceFacing.sumpos,0) + NVL(lastFace.sumpos,0) ) - NVL(eventLoc.sumpos,0) 
                                      / ( NVL(choiceDir.sumpos,0) + NVL(eventLoc.sumpos,0) + NVL(choiceFacing.sumpos,0) + NVL(lastFace.sumpos,0) )
                              ELSE 0 END
                       + CASE WHEN ( NVL(choiceFacing.sumpos,0) <> 0 ) AND ( NVL(choiceFacing.sumpct,0) <> 0 ) 
                              THEN ( NVL(choiceFacing.pos,0) * NVL(choiceFacing.pct,0) ) / ( NVL(choiceFacing.sumpos,0) * NVL(choiceFacing.sumpct,0) ) 
                                      * ( NVL(choiceDir.sumpos,0) + NVL(eventLoc.sumpos,0) + NVL(choiceFacing.sumpos,0) + NVL(lastFace.sumpos,0) ) - NVL(choiceFacing.sumpos,0) 
                                      / ( NVL(choiceDir.sumpos,0) + NVL(eventLoc.sumpos,0) + NVL(choiceFacing.sumpos,0) + NVL(lastFace.sumpos,0) )
                              ELSE 0 END
                       + CASE WHEN ( NVL(lastFace.sumpos,0) <> 0 ) AND ( NVL(lastface.sumpct,0) <> 0 ) 
                              THEN ( NVL(lastFace.pos,0) * NVL(lastFace.pct,0) ) / ( NVL(lastFace.sumpos,0) * NVL(lastFace.sumpct,0) )
